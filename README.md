...
--- Нет возможности подключить HSE - только HSE_IN от генератора
--- Значение параметра HSI_FS также может принимать значения b000,b001,b010,b011
      ...
      это позволяет на старте задавать частоту HSI = 2.1MHz
--- Калибровочная константа на 48 МГц присутствует, но частота не соответствует
      (скорее всего не тестируют как для PY32L020)         
--- Flash Sleep Mode - для выхода из режима в примерах создана функция в startup (ASM)
      (тестовый вариант - проверяется)  
      (потребление - LP Run + Flash Sleep - 100-120 мкА )   
--- По сравнению с серией PY32F002A+ - работа с RAM быстрее ( по 2 такта на команды LDR/STR )      
...
Документация RM  более точная от PY32L020  - англ. версия 1.3
